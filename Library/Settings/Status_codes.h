...
#define STATUS_SPI_ERRORS       (STATUS_SPI_DISPLAY_INIT_ERROR  | STATUS_SPI_DISPLAY_TX_ERROR   | STATUS_SPI_DISPLAY_NO_RESPONSE)
#define STATUS_EEPROM_ERRORS    (STATUS_EEPROM_INIT_ERROR         | STATUS_EEPROM_WRITE_ERROR       | STATUS_EEPROM_READ_ERROR       | STATUS_EEPROM_CRC_ERROR)
#define STATUS_ADC_ERRORS       (STATUS_ADC_EXTERNAL_INIT_ERROR   | STATUS_ADC_EXTERNAL_SENSOR_ERROR| STATUS_ADC_BOARD_TEMP_ERROR    | STATUS_ADC_RANGE_ERROR)
#define STATUS_TEMP_SENSOR_ERRORS (STATUS_TEMP_SENSOR_COMM_ERROR | STATUS_TEMP_SENSOR_RANGE_ERROR)
#define STATUS_RS485_ERRORS     (STATUS_RS485_INIT_ERROR          | STATUS_RS485_RX_ERROR           | STATUS_RS485_TX_ERROR)
#define STATUS_USB_ERRORS       (STATUS_USB_INIT_ERROR            | STATUS_USB_UART_CMD_ERROR       | STATUS_USB_FLASH_WRITE_ERROR   | STATUS_USB_RESERVED_ERROR)
#define STATUS_FLASH_ERRORS     (STATUS_FLASH_INIT_ERROR          | STATUS_FLASH_WRITE_ERROR        | STATUS_FLASH_READ_ERROR        | STATUS_FLASH_CRC_ERROR)
#define STATUS_SD_ERRORS        (STATUS_SD_INIT_ERROR             | STATUS_SD_MOUNT_ERROR           | STATUS_SD_WRITE_ERROR          | STATUS_SD_READ_ERROR           | \
                                 STATUS_SD_CORRUPTED_DATA         | STATUS_SD_CRC_MISMATCH          | STATUS_SD_FILE_OPEN_ERROR      | STATUS_SD_TEMP_OUT_OF_RANGE)
#define STATUS_GSM_ERRORS       (STATUS_GSM_REG_ERROR             | STATUS_NBIOT_REG_ERROR          | STATUS_GSM_NET_ERROR           | STATUS_GSM_NO_SIM              | \
                                 STATUS_UART_SMS_SEND_ERROR       | STATUS_UART_SERVER_COMM_ERROR   | STATUS_UART_SERVER_UNAVAILABLE | STATUS_UART_NO_RESPONSE        | \
                                 STATUS_UART_WRONG_PASSWORD)
#define STATUS_FIRMWARE_ERRORS  (STATUS_FIRMWARE_UPDATE_ERROR     | STATUS_FIRMWARE_UPDATE_INTERRUPTED | STATUS_FIRMWARE_CHECK_ERROR)
#define STATUS_COMMON_ERRORS    (STATUS_IDLE_LOOP_MODE            | STATUS_INTERNAL_ERROR           | STATUS_VOLTAGE_TOO_LOW         | STATUS_VOLTAGE_TOO_HIGH)
#define STATUS_CRITICAL_ERRORS  (STATUS_CRITICAL_ERROR            | STATUS_UNKNOWN_ERROR            | STATUS_STACK_OVERFLOW          | STATUS_HARDFAULT_OCCURRED)
#define STATUS_W25Q_ERRORS      (STATUS_W25Q_INIT_ERROR           | STATUS_W25Q_WRITE_ERROR         | STATUS_W25Q_ERASE_ERROR        | STATUS_W25Q_TIMEOUT)
#define STATUS_FAULTS           (STATUS_NMI_OCCURRED              | STATUS_MEMMANAGE_FAULT          | STATUS_BUSFAULT_OCCURRED       | STATUS_USAGEFAULT_OCCURRED)
