...
/* Объединяем аппаратные ошибки приема (паритет, фрейминг, оверран) в один флаг */
...
/* ---------- 6. ОШИБКИ USB (биты 16..19) ----------
   Вернули подробные коды USB, как было изначально. */
...
/* ---------- 8. ОШИБКИ SD-КАРТЫ (биты 24..30 и 35) ----------
   Убраны ошибки создания файлов/директорий и "файл не найден".
   Оставлены: */
...
/* Биты 31..34 будут использоваться для W25Q (см. ниже) */

/* Оставляем SD ошибку по температуре на отдельном бите */
#define STATUS_SD_TEMP_OUT_OF_RANGE        (1ULL << 35)  // Температура вне допустимого диапазона для SD

/* ---------- 9. ОШИБКИ GSM (биты 36..44) ---------- */
#define STATUS_GSM_REG_ERROR               (1ULL << 36)  // Ошибка регистрации в сети GSM
#define STATUS_NBIOT_REG_ERROR             (1ULL << 37)  // Ошибка регистрации в сети NB-IoT
#define STATUS_GSM_NET_ERROR               (1ULL << 38)  // Ошибка выхода в интернет по GSM
#define STATUS_GSM_NO_SIM                  (1ULL << 39)  // SIM-карта отсутствует
#define STATUS_UART_SMS_SEND_ERROR         (1ULL << 40)  // Ошибка отправки SMS
#define STATUS_UART_SERVER_COMM_ERROR      (1ULL << 41)  // Ошибка связи с сервером
#define STATUS_UART_SERVER_UNAVAILABLE     (1ULL << 42)  // Сервер недоступен
#define STATUS_UART_NO_RESPONSE            (1ULL << 43)  // Нет ответа от сервера/устройства
#define STATUS_UART_WRONG_PASSWORD         (1ULL << 44)  // Неверный пароль

/* ---------- 10. ОШИБКИ ПРОШИВКИ (биты 45..47) ---------- */
#define STATUS_FIRMWARE_UPDATE_ERROR       (1ULL << 45)  // Ошибка обновления (нарушена целостность прошивки)
#define STATUS_FIRMWARE_UPDATE_INTERRUPTED (1ULL << 46)  // Обновление прервано
#define STATUS_FIRMWARE_CHECK_ERROR        (1ULL << 47)  // Ошибка проверки прошивки (rollback или неверная подпись)

/* ---------- 11. ОБЩИЕ ОШИБКИ (биты 48..49) ---------- */
#define STATUS_IDLE_LOOP_MODE              (1ULL << 48)  // Переход в idle loop из-за долгого бездействия
#define STATUS_INTERNAL_ERROR              (1ULL << 49)  // Внутренняя ошибка системы

/* ---------- 12. КРИТИЧЕСКИЕ ОШИБКИ (биты 50..53) ---------- */
#define STATUS_CRITICAL_ERROR              (1ULL << 50)  // Критическая ошибка системы
#define STATUS_UNKNOWN_ERROR               (1ULL << 51)  // Неизвестная ошибка системы
#define STATUS_STACK_OVERFLOW              (1ULL << 52)  // Переполнение стека (RTOS/система)
#define STATUS_HARDFAULT_OCCURRED          (1ULL << 53)  // Произошёл HardFault (или BusFault, если не обработан отдельно)

/* ---------- 13. ОШИБКИ W25Q (биты 31..34) ----------*/
#define STATUS_W25Q_INIT_ERROR             (1ULL << 31)  // Не удалось инициализировать W25Q
#define STATUS_W25Q_WRITE_ERROR            (1ULL << 32)  // Ошибка записи в W25Q (Page Program)
#define STATUS_W25Q_ERASE_ERROR            (1ULL << 33)  // Ошибка стирания в W25Q (Sector/Block Erase)
#define STATUS_W25Q_TIMEOUT                (1ULL << 34)  // Превышен тайм-аут W25Q (Busy не снялся)

/* ---------- 14. ИСКЛЮЧИТЕЛЬНЫЕ ФОЛТЫ (FAULTS) (биты 54..57) ----------*/
#define STATUS_NMI_OCCURRED                (1ULL << 54)  // Произошёл NMI (Non-Maskable Interrupt)
#define STATUS_MEMMANAGE_FAULT             (1ULL << 55)  // Произошёл MemManage Fault (ошибка управления памятью)
#define STATUS_BUSFAULT_OCCURRED           (1ULL << 56)  // Произошёл BusFault (ошибка доступа к памяти или периферии)
#define STATUS_USAGEFAULT_OCCURRED         (1ULL << 57)  // Произошёл UsageFault (неопределённая инструкция или иное)

/* ---------- 15. РЕЗЕРВ (Оставшиеся биты 58..63) ---------- */
#define STATUS_RESERVED_58                 (1ULL << 58)
#define STATUS_RESERVED_59                 (1ULL << 59)
