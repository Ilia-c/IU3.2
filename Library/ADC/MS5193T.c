...
void calculate_ADC_data_heigh(int32_t adValue, uint8_t channel) {
    ...
    ADC_data.ADC_Current[channel] = (double)ADC_Current_ld;
    ...
    long double H_correct = ADC_SI_value_ld - EEPROM.ZERO_LVL[channel];
    long double H_Imm = H_correct * Imax_Imin;
    long double ADC_Current_new_ld = fmal((H_Imm / VPI_NPI), 1.0L, DEFAULT_GVL_CORRECT_4M);
    ...
    snprintf(ADC_data.ADC_Current_char[channel], sizeof(ADC_data.ADC_Current_char[channel]), "%4.3f", ADC_Current_new);
}
